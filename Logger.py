...
class Logger(object):
    def __init__(self, fields):
        ...
        if (type(fields) != list):
            raise Exception(
                '[Logger.__init__] Attribute "fields" must be a list and each entry should contain a dictionary with attributes "id", "name", "headerformat" and "valueformat"')
        ...
        for field in fields:
            ...
            if (type(field['headerformat']) != str): raise Exception(
                '[Logger.__init__] Field "headerformat" must be a string')
            if (type(field['valueformat']) != str): raise Exception(
                '[Logger.__init__] Field "valueformat" must be a string')
    ...
    def printValues(self, fieldValues):
        if (type(fieldValues) != dict):
            raise Exception(
                '[Logger.printValues] Attribute "fieldValues" must be a dictionary indexed by field id and the field value as value')
